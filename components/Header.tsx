...
const Header: React.FC<HeaderProps> = ({ user, language, onLanguageChange, onMenuClick, onLogout, onNavigate }) => {
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-100 sticky top-0 z-30 px-4 md:px-8 flex items-center justify-between shadow-sm">
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        {/* Profile Section */}
        <div className="flex items-center gap-3 pl-3 md:pl-4 border-l border-gray-100">
          ...
          {/* Notifications */}
          <Notifications user={user} onNavigate={onNavigate} language={language} />

          {/* Profile Dropdown */}
          <div className="relative" ref={profileRef}>
            <button 
              onClick={() => setShowProfile(!showProfile)}
              className="w-9 h-9 md:w-10 md:h-10 bg-secondary/10 rounded-full border-2 border-secondary/20 flex items-center justify-center text-secondary font-bold"
            >
            ...
            {showProfile && (
              <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-100 rounded-xl shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                ...
                <button onClick={onLogout} className="w-full px-4 py-2.5 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-3 font-bold">
